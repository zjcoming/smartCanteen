 ...
 * @Description 线程池和线程池管理类   任务是什么类型获取什么样的线程池就行
 *
 * getmUPLoadPool()      获取下载任务的线程池
 * getMshortPool()       获取短时间任务的线程池（长短任务分开，防止短任务被饿死
 * getmLoogPool)()       获取长时间任务的线程池
 * getSinglePool()       获取一个单线程池,所有任务都会按照假如的顺序执行,避免同步开销的问题
 *
 * 使用方式：Java示例
 ...
 *         };
 *
 *         //执行任务(任务是什么类型获取什么样的线程池就行）
 *         ThreadManager.getmUPLoadPool().execute(downloadTask);
 *
 */
